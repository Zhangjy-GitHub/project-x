...
def generate_special_quarter_confirm_form(service_company: str, year: str, quarter: str, confirm_info: dict):
    ...
    r = 4
    for (service_type, confirm_detail) in confirm_info.items():
        if r >= len(table.rows):
            table.add_row()
    ...
    table.rows[r].cells[1].merge(table.rows[r].cells[-1])
    content = '协作商：' + service_company + '（盖章）'
    p = table.rows[r].cells[1].add_paragraph(content)
    p.runs[0].font.size = Pt(12)
    p.runs[0].font.bold = True
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE

    p = table.rows[r].cells[1].add_paragraph('负责人（签字）：')
    p.runs[0].font.size = Pt(12)
    p.runs[0].font.bold = True
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = table.rows[r].cells[1].add_paragraph('    年   月   日')
    p.runs[0].font.size = Pt(12)
    p.runs[0].font.bold = True
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
