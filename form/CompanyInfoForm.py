...
def generate_company_info_form(companies_info: dict):
    ...
    info_poisons = {'company': (0, 1), 'person': (0, 7), 'register_address': (1, 1), 'register_fund': (2, 1),
                    'founded_time': (2, 4), 'employee_total': (2, 8), 'business_scope': (3, 1), 'account_bank': (4, 1),
                    'account_num': (4, 7), 'contact_person': (5, 1), 'contact_phone': (5, 7),
                    'bank_name': (4, 1), 'bank_account': (4, 7)}
    ...
    for p in check_poisons:
        ...
        pr.add_run(u'\u2713' + '无。□有：')
        run.font.size = Pt(12)
    save_company_form_file(company=companies_info['company'], year=None, form_name='1 供应商情况', doc=doc)
