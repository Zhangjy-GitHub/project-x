...
def parse_html(html):
    ...
    first_row = page.xpath('//section[@id="Cominfo"]/table[@class="ntable"]//tr')[0]
    founded_time = first_row[5].xpath('text()')[0].strip()
    second_row = page.xpath('//section[@id="Cominfo"]/table[@class="ntable"]//tr')[1]
    register_fund = second_row[1].xpath('text()')[0].strip()
    fifth_row = page.xpath('//section[@id="Cominfo"]/table[@class="ntable"]//tr')[5]
    employee_total = fifth_row[1].xpath('text()')[0].strip()
    sixth_row = page.xpath('//section[@id="Cominfo"]/table[@class="ntable"]//tr')[7]
...
if __name__ == '__main__':
    ...
    company_info = {}
    headers = {
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/536.5 (KHTML, like Gecko) Chrome/19.0.1084.9 '
                      'Safari/536.5'}
    error_urls = []
